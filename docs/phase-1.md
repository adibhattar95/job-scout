...
1. Foundation and onboarding: Phoenix, Ollama adapter, validated profiles/preferences, run records, durable quota primitive. Initial implementation and latest-candidate reload are complete. Text-based PDF upload is implemented; scanned PDFs need OCR.
2. Resume ingestion: PDF text extraction and clear scanned-PDF errors implemented; source-addressable facts, user-confirmed additions, profile history/delete still pending.
3. Discovery: first-pass Remotive and optional JSearch adapters, 24-hour source cache, duplicate removal, source diagnostics, one role/location query per click, and a conservative rolling JSearch cap are implemented. Broader sources and richer query planning remain.
...
PDF or pasted text input, 16,000-character extracted-text limit, local JSON snapshots, no candidate history UI yet. Evidence substring checks catch invented quotations but not unsupported paraphrases. Search currently covers Remotive remote jobs and JSearch when configured; relevance is a title/skill heuristic, not a validated fit score. No tailoring, PDF export, or baseline quality claim yet.
